...
function displayRecord(start, finish, arrays) {
    ...
    if(finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            var actions = "<a href='/Organizations/SMS/View/"+organization_details.organization_id+"/"+arrays[index][4]+"' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='View this message' style=\"padding: 3px;\"><span class=\"d-inline-block border border-white w-100 text-center\" style=\"border-radius: 2px; padding: 5px;\"><i class='ft-eye'></i> View</span></a>";
            tableData += "<tr><th scope='row'><input type='checkbox' class='actions_id' id='actions_id_"+arrays[index][4]+"'><input type='hidden' id='actions_value_"+arrays[index][4]+"' value='"+arrays[index][4]+"'> "+counter+"</th><td>"+arrays[index][8] +" "+status+"<br><small><a class='text-secondary' href='/Organization/ViewClient/"+organization_details.organization_id+"/"+arrays[index][0]+"'>{"+arrays[index][7] +"}</a></small></td><td><span data-toggle='tooltip' data-html='true' title='"+arrays[index][3]+"'>"+message+"</span></td><td>"+transactiontype+"</td><td>"+actions+"</td></tr>";
            counter++;
        }
    }else{
        //create a table of the 50 records
