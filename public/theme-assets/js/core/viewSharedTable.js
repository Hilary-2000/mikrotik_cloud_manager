...
function displayRecord(start, finish, arrays) {
    ...
    if(finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            var actions = "<a href='/SharedTables/Edit/" + tables_data.table_id + "/Name/"+replacePunctuationWithUnderscore(tables_data.table_name)+"/Record/"+arrays[index][(arrays[index].length - 1)]+"' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='View this shared table!' style=\"padding: 3px;\"><span class=\"d-inline-block border border-white w-100 text-center\" style=\"border-radius: 2px; padding: 5px;\"><i class='ft-eye'></i> View</span></a>";
