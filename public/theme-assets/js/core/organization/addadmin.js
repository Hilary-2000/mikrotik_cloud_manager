...
function displayRecord(start, finish, arrays) {
    ...
    if(finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            tableData += "<tr><th scope='row'>"+counter+"</th><td>" + arrays[index][1] +" "+status+"</td><td>" + arrays[index][2] + "</td><td>" + arrays[index][8] + "</td><td>" + arrays[index][6] + "</td><td><a href='/Accounts/Admin/View/"+arrays[index][0]+"' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='View this User'><i class='ft-eye'></i></a> <a href='/Accounts/Admin/Disable/"+arrays[index][0]+"' class='btn btn-sm btn-warning text-bolder "+readonly_flag+"'  data-toggle='tooltip' title='Disable this User'><i class='ft-alert-octagon'></i></a></td></tr>";
            counter++;
        }
    }else{
        //create a table of the 10 records
