...
window.onload = function () {
    ...
    let table = $('#clients_table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "/Organization/Clients/datatable/"+organization_details.organization_id, // route to controller
            type: "GET",
            data: function (d) {
...
function displayRecord(start, finish, arrays) {
    ...
    if (finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            var actions = "<a href='/Organization/ViewClient/"+organization_details.organization_id+"/" + arrays[index][0] + "' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='View this User' style=\"padding: 3px;\"><span class=\"d-inline-block border border-white w-100 text-center\" style=\"border-radius: 2px; padding: 5px;\"><i class='ft-eye'></i> View</span></a>";
            tableData += "<tr><th scope='row'><input type='checkbox' class='actions_id' id='actions_id_"+arrays[index][11]+"'><input type='hidden' id='actions_value_"+arrays[index][11]+"' value='"+arrays[index][11]+"'> " + counter + "</th><td>" + assignment + " <a href='/Organization/ViewClient/"+organization_details.organization_id+"/" + arrays[index][0] + "' class='text-secondary'>" + ucwords(arrays[index][1]) + " " + status + "</a><br><small class='text-gray d-none d-xl-block'>" + ucword(arrays[index][13]) + "</small></td><td>" + arrays[index][11].toUpperCase() + " " + reffered + "</td><td>" + ucwords(arrays[index][5]) + "<br><small class='d-none d-md-block'>" + location + "</small></td><td>" + setDate(arrays[index][7]) + "</td><td>"+actions+"</td></tr>";
            counter++;
        }
    } else {
        //create a table of the 50 records
