...
function displayRecord(start, finish, arrays) {
    ...
    if(finish < total) {
        ...
        for (let index = start; index < finish; index++) {
            ...
            tableData += "<tr data-toggle='tooltip' title='Paid Ksh "+arrays[index][2]+" to "+arrays[index][4]+" {"+arrays[index][10]+"} using "+arrays[index][11]+" {"+arrays[index][0]+"} on "+arrays[index][12]+".'><th scope='row'>"+counter+"</th><td>" + arrays[index][7] +" "+status+"<small>{"+arrays[index][12]+"}</small></td><td>" + arrays[index][4] + " <small><a href='/Organization/View/"+arrays[index][3]+"' class='text-secondary'>{"+arrays[index][10]+"}</a></small></td><td>Kes " + arrays[index][2] + "</td><td><a href='/Transactions/View/" + arrays[index][6] + "' class='btn btn-sm btn-primary text-bolder' data-toggle='tooltip' title='View this transaction'><i class='ft-eye'></i> View</a></td></tr>";
            counter++;
        }
    }else{
        //create a table of the 50 records
