...
<<<<<<< HEAD
        ...
        // send money
        $result = DB::select("SELECT * FROM `admin_table_mikrotik_clouds` WHERE `admin_username` = '$username' AND `admin_password` = '$password'");
        if (count($result) > 0) {
            ...
            $req->session()->put("priviledges",$result[0]->priviledges_excempted);
...
>>>>>>> origin/main
        ...
        if (count($verify) > 0) {
            ...
            // get the user data
<<<<<<< HEAD
            $user_data = DB::select("SELECT * FROM `admin_table_mikrotik_clouds` WHERE `admin_id` = '$user_id'");
            ...
            if (count($user_data) > 0) {
                ...
                DB::table("admin_table_mikrotik_clouds")->where("admin_id",$user_id)->update(["last_time_login" => date("YmdHis"),'date_changed' => date("YmdHis")]);
